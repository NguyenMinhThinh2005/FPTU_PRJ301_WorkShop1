...
public class DeleteTransactionController extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            if (dao.delete(transactionId)) {
                request.setAttribute("MSG", "Transaction deleted successfully.");
            } else {
                request.setAttribute("MSG", "Failed to delete transaction.");
            }
            request.setAttribute("list", dao.search(""));
            request.getRequestDispatcher("transactionList.jsp").forward(request, response);
        } catch (NumberFormatException e) {
            request.setAttribute("MSG", "Invalid transaction ID format.");
            request.getRequestDispatcher("transactionList.jsp").forward(request, response);
        } catch (Exception e) {
            log("Error at DeleteTransactionController: " + e.toString());
            request.setAttribute("MSG", "An error occurred while deleting the transaction.");
            request.getRequestDispatcher("transactionList.jsp").forward(request, response);
        }
    } 
